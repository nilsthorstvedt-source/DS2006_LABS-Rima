...
while rounds_played < total_rounds:
    ...
    for i in range(len(players)):
        ...
        total += roll_d6
        print(f"{name} rolled a D6: {roll_d6}")

        roll_d12 = dice_collection.D12()
        total += roll_d12
        print(f"{name} rolled a D12: {roll_d12}")
...
max_score = max(player[1] for player in players)
champions = [player[0] for player in players if player[1] == max_score]
